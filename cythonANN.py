...
def benchmark_gradient():
    """ Benchmark Python vs Cython gradient computation """
    ...
    print("\nBenchmarking Python vs Cython Optimized Gradient Calculation:\n")

    # Benchmark the original Python function
    start_time = time.perf_counter()
    grad_py = gradient(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda)
    py_time = time.perf_counter() - start_time
    print(f"Python gradient computation time: {py_time:.6f} seconds")

    # Benchmark the Cython-optimized function
    start_time = time.perf_counter()
    grad_cy = cythonANN.compute_gradient(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda)
    cy_time = time.perf_counter() - start_time
    print(f"Cython gradient computation time: {cy_time:.6f} seconds")

    # Compare speedup
    speedup = py_time / cy_time
    print(f"Speedup using Cython: {speedup:.2f}x")
