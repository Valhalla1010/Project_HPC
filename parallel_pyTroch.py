...
def predict(Theta1, Theta2, X):
	...
	a2 = torch_sigmoid(a1 @ torch_Theta1)                 # apply sigmoid: input layer --> hidden layer
	a2 = torch.hstack((torch.ones((m,1)), a2))  # add bias (hidden layer)
	a3 = torch_sigmoid(a2 @ torch_Theta2)                 # apply sigmoid: hidden layer --> output layer
...
def cost_function(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
	...
	a2 = torch_sigmoid(a1 @ torch_Theta1)                 
	a2 = torch.hstack((torch.ones((m,1)), a2))  
	a3 = torch_sigmoid(a2 @ torch_Theta2)
...
def main():
	...
	theta = optimize.fmin_cg(cost_function, theta0, fprime=gradient, args=args, callback=cbf, maxiter=150)
...
if __name__ == "__main__":
    ...
    print(f"Execution Time Before Parallel: {execution_time_before} seconds")
    
    start_time_parallel = timer()
    main()  # Run the main with multi-threading optimization
    end_time_parallel = timer()
    execution_time_after = end_time_parallel - start_time_parallel
    speedup = (execution_time_before - execution_time_after) / execution_time_before * 100
    print(f"Execution Time After Parallel: {execution_time_after} seconds")
    print(f"Speedup: {speedup:.2f}% Improvement")